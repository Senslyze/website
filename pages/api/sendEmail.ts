...
export default async function handler(
  ...
  let firstName; // Declare firstName outside the block
  let lastName;
  let email;
  let phoneNo;
  let message;
  let countryCode;
  const yourname = "Aditya Mande";
  const professionaltitle = "Operations Manager";
  const contact = "+919834044166";

  if (req.method === "POST") {
    // Destructure more fields from the request body
    ({ firstName, lastName, email, phoneNo, message, countryCode } = req.body);
